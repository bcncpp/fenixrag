...
@app.command()
def add(content: str, title: str | None = None, source: str | None = None):
    """Add a single document"""

    async def _add_doc():
        doc_service = DocumentService()
        doc_id = await doc_service.add_document(content=content, title=title, source=source or "manual_input")
            ...
            if command == "search" and len(parts) > 1:
                ...
                with console.status(f"Filtering {category} for: {query}"):
                    docs_with_scores = await doc_service.similarity_search(
                        query, limit=5, filters={"category": category}
                    )
